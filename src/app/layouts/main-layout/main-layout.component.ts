    ...
    <div class="min-h-screen bg-gray-50 flex">
      ...
      <aside [class]="getSidebarClass()">
        ...
        <nav class="flex-1 p-4 space-y-1">
          ...
          <a routerLink="/dashboard" 
             [routerLinkActive]="isAdmin() ? 'bg-slate-700' : 'bg-indigo-800'"
             class="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-opacity-50 transition-colors"
        ...
        <div class="p-4 border-t" [class.border-indigo-800]="!isAdmin()" [class.border-slate-700]="isAdmin()">
          <div class="px-4 py-2 mb-2">
            ...
            <p class="text-xs truncate" [class.text-indigo-300]="!isAdmin()" [class.text-slate-400]="isAdmin()">{{ authService.currentUser()?.email }}</p>
          </div>
          <button (click)="logout()"
                  class="w-full flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-opacity-50 transition-colors text-left text-sm"
                  [class]="isAdmin() ? 'hover:bg-slate-700' : 'hover:bg-indigo-800'">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header class="bg-white border-b border-gray-200 px-6 py-4">
          <h2 class="text-xl font-semibold text-gray-900">{{ isAdmin() ? 'Panel de Administración' : 'Panel de Control' }}</h2>
        </header>
...
export class MainLayoutComponent {
  ...
  getSidebarClass(): string {
    return this.isAdmin() 
      ...
      : 'w-64 bg-indigo-900 text-white flex flex-col flex-shrink-0';
  }
